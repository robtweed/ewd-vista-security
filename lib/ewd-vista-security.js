...
module.exports = {
  ...
  initiate: function(appName, ewd) {
    ...
    if (ewdChild.Custom && ewdChild.Custom.encryptAVCode === false) encrypt = false;
    if (encrypt) {
  ...
  login: function(ewd, session) {
    ...
    var credentials;
    var cipher;
    var decrypt = true;  // AVCode will have been encrypted unless explicitly over-ridden
    if (ewdChild.Custom && ewdChild.Custom.encryptAVCode === false) decrypt = false;
    if (decrypt) {
